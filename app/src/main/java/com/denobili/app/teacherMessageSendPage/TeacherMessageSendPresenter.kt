...
import com.denobili.app.teacherOutboxPage.StudentIdClass
        ...
        if (NetworkHelper.isOnline(context)) {
            when (sendModel.feature) {
                ...
                Selected_Feature.NOTIFICATION_FEATURE.`val` -> {
                    ...
                    messagingInteractor.postNotification(NotifySendModel(sendModel.subject!!,
                ...
                Selected_Feature.EMAIL.`val` -> {
                    ...
                    messagingInteractor.postMail(EmailSendModel(sendModel.subject!!,
                            sendModel.message!!, null, null, sendModel.list!!, sendModel.attachment_id, sendModel.class_id, getCurrentDate(), sendModel.scheduleTime)).subscribe(observer())
                }

                Selected_Feature.DIARY.`val` -> {
                    messagingInteractor.postDiary(DiarySendModel(sendModel.subject!!,
                            sendModel.message!!,
                            null, null,
