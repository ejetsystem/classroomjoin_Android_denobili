...
import com.denobili.app.invoice.trans_detail.TransDetail
...
import kotlinx.android.synthetic.main.invoice_detail.*
import org.jetbrains.anko.AnkoLogger
import rx.Observer

class InvoiceDetailActivity : BaseListActivity(), AnkoLogger {

    private var adapter: InvoiceDetailAdapter? = null
    private var adapter1: InvoiceDetail1Adapter? = null

    private var presenter: InvoiceDetPresenter? = null
    private var progressdialog: ProgressDialog? = null
    private var type: String = "1"
    private var invoice_id:Int?=null
    private var fees:String?=null
    private var amountFee:String?=null
    private var recipient:String?=null
    private var adjsutment:String?=null
    private var discount:String?=null
    private var status:String?=null
    private var paymentD:String?=null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        transactionD.setOnClickListener{
            val intent=Intent(this,TransDetail::class.java)
            intent.putExtra("listTrans",transList)
            startActivity(intent)
        }
        invoice_id=intent.getIntExtra("invoice_id",0)
        recipient=intent.getStringExtra("recipient")
        fees=intent.getStringExtra("fee_month")
        discount=intent.getStringExtra("discount")
        status=intent.getStringExtra("status")
        paymentD=intent.getStringExtra("paymentDetail")
        amountFee=intent.getStringExtra("amount")
        adjsutment=intent.getStringExtra("adjustment")
        inNum.setText(invoice_id.toString())
        invoiceD.setText("Invoice-#"+invoice_id.toString())
        recNum.setText(recipient)
        inAm.setText(amountFee)
        feeDate.setText(fees)
        receipt.setText(recipient)


        if (status.equals("1"))
            fee_status.setText("Paid")
        else
            fee_status.setText("Unpaid")
...
*/
        MainBus.getInstance().busObservable.ofType(ConnectInvoice::class.java).subscribe(eventobserver)
        presenter = InvoiceDetPresenter(this,invoice_id.toString())

        adapter1 = InvoiceDetail1Adapter(null, this)
        adapter = InvoiceDetailAdapter(null, this)
        anuual_fee.adapter=adapter1
        anuual_fee.layoutManager=LinearLayoutManager(this)
        anuual_fee.isNestedScrollingEnabled=false

        recyclerView.adapter = adapter
        recyclerView.isNestedScrollingEnabled = false
        progressdialog = DialogUtil.showProgressDialog(this@InvoiceDetailActivity)
...
*/
    ...
    private val eventobserver = object : Observer<ConnectInvoice> {
        ...
        override fun onNext(event: ConnectInvoice) {
            if (swipeContainer.isRefreshing) swipeContainer.isRefreshing = false
            if (progressdialog!!.isShowing) progressdialog?.dismiss()
            when (event.event) {
                Event.RESULT -> {
                    transList= event.result as ArrayList<DisplayItem>?
                    if (transList?.size==0){
                        transactionD.visibility=View.GONE
                    }else{
                        transactionD.visibility=View.VISIBLE
                    }
                    adapter1!!.addItems(true,event.res)
    ...
    private fun getData() {
        if (NetworkHelper.isOnline(this@InvoiceDetailActivity)) {
    ...
    private fun showdeletealert(position: Int) {
        var alert = AlertDialog.Builder(this@InvoiceDetailActivity)
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        if (item!!.itemId == android.R.id.home) {
            ...
            startActivity(Intent(this@InvoiceDetailActivity, LandingActivity::class.java).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP))
            finishAffinity()
        }
        return true
    }
    ...
    private fun setUpItemTouchHelper() {
        ...
        val simpleItemTouchCallback = object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            init {
                ...
                xMark = AppCompatResources.getDrawable(this@InvoiceDetailActivity, R.drawable.ic_delete_green_24dp)
                // xMark =  VectorDrawableCompat.create(getResources(), R.drawable.ic_delete_green_24dp_data, null);
                // xMark = AppCompatDrawableManager.get().getDrawable(this@InvoiceDetailActivity, R.drawable.ic_delete_green_24dp_data);
