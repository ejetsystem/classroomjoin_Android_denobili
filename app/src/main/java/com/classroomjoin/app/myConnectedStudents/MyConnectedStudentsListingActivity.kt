...
import com.classroomjoin.app.studentCodePage.InputStudentCode
import com.classroomjoin.app.studentHomePage.Student_Home_Activity
...
class MyConnectedStudentsListingActivity : BaseListActivity(), ForceUpdateChecker.OnUpdateNeededListener {

    private var adapter: ConnectedStudentAdapter? = null
    private var presenter: ConnectedStudentPresenter? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        presenter = ConnectedStudentPresenter(this)

        adapter = ConnectedStudentAdapter(null, this)
        recyclerView.adapter = adapter
        recyclerView.isNestedScrollingEnabled = false
        progressdialog = DialogUtil.showProgressDialog(this@MyConnectedStudentsListingActivity)

        // fab.imageResource = R.drawable.ic_person_add_white_24dp
        fab.onClick {
            startActivity<InputStudentCode>()
        }
        setUpItemTouchHelper()

        ItemClickSupport.addTo(recyclerView).setOnItemClickListener { recyclerView, position, v ->
            val model: MyConnectedStudentModel = adapter?.getItem(position) as MyConnectedStudentModel
...
*/
                ...
                Event.RESULT -> {
                    ...
                    /*if(event.results!!.size==1){
                      ...
                      //  val selectedView = recyclerView.getChildAt(0)
                        ...
                        val model: MyConnectedStudentModel = adapter?.getItem(0) as MyConnectedStudentModel
                        presenter?.register(model)
                        startActivity<Student_Home_Activity>()
                       // finish()
                    }*/
    ...
    private fun getData() {
        if (NetworkHelper.isOnline(this@MyConnectedStudentsListingActivity)) {
    ...
    private fun showdeletealert(position: Int) {
        var alert = AlertDialog.Builder(this@MyConnectedStudentsListingActivity)
        ...
        alert.setPositiveButton(getString(R.string.btn_yes)) { dialog, whichButton ->
            studentid = (adapter?.getItem(position) as MyConnectedStudentModel).id.toString()
            deletStudent()
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        if (item!!.itemId == android.R.id.home) {
            ...
            startActivity(Intent(this@MyConnectedStudentsListingActivity, LoginActivity::class.java).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP))
            finish()
        }
        return true
    }
    ...
    private fun setUpItemTouchHelper() {
        ...
        val simpleItemTouchCallback = object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            init {
                ...
                xMark = AppCompatResources.getDrawable(this@MyConnectedStudentsListingActivity, R.drawable.ic_delete_green_24dp)
                // xMark =  VectorDrawableCompat.create(getResources(), R.drawable.ic_delete_green_24dp_data, null);
                // xMark = AppCompatDrawableManager.get().getDrawable(this@MyConnectedStudentsListingActivity, R.drawable.ic_delete_green_24dp_data);
